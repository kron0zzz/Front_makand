...
export const useSuppliers = () => {
  ...
  const cargarProveedores = useCallback(async () => {
    setLoading(true);
    try {
      const datos = await supplierService.obtenerTodos();
      setSuppliers(datos);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const toggleProveedorEstado = async (id, estadoActual, proveedorCompleto) => {
    if (!window.confirm(`¿Estás seguro de que deseas ${estadoActual ? 'desactivar' : 'activar'} este proveedor?`)) {
      return;
    }

    try {
      const datosActualizados = { ...proveedorCompleto, supplier_status: !estadoActual };
      await supplierService.actualizar(id, datosActualizados);
