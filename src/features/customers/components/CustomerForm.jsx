...
const CustomerForm = ({ isOpen, onClose, formData, setFormData, isEditing }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditing 
      ? `http://localhost:3000/api/customers/${formData.client_id}` 
    ...
    try {
      ...
      if (response.ok) {
        alert(isEditing ? 'Cliente actualizado' : 'Cliente creado con éxito');
        await cargarClientes(); // Refrescar la tabla
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
      <div style={{ backgroundColor: 'white', borderRadius: '12px', width: '90%', maxWidth: '750px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 30px' }}>
            ...
            {/* ID Cliente (Visible solo en edición) */}
            <div>
              <label style={labelStyle}>ID Cliente</label>
              <input 
                type="text" 
                style={{ ...inputStyle, backgroundColor: '#f3f4f6', cursor: 'not-allowed' }} 
                value={isEditing ? `#${formData.client_id}` : 'Autogenerado'} 
                readOnly 
              />
            </div>
          ...
          <div style={{ marginTop: '32px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
            <button type="button" onClick={onClose} style={{ padding: '10px 20px', borderRadius: '8px', border: '1px solid #d1d5db', backgroundColor: 'white', fontWeight: '600', cursor: 'pointer' }}>
              Cancelar
            </button>
            <button type="submit" style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', backgroundColor: '#ff6b35', color: 'white', fontWeight: '600', cursor: 'pointer' }}>
              {isEditing ? 'Actualizar' : 'Guardar'}
            </button>
          </div>
        </form>
