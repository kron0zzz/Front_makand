...
//                           //const pedido = await abrirWorkspace(order.order_id);

//                           onOpenWorkspace(order.order_id);
...
const OrderPage = ({ onOpenWorkspace }) => {
  ...
  const pedidosFiltrados = useMemo(() => {
    const datos = Array.isArray(orders) ? orders : [];
    const termino = busqueda.toLowerCase();

    return datos.filter((order) => {
      const proyecto = order.project_name?.toLowerCase() || "";
      const cliente = `${order.customer_name || ""}`.toLowerCase();
      const pedido = order.order_id?.toString() || "";
